...
class XDeckNav extends HTMLElement {
  ...
  get #rowSize() {
    return this.#cache.getOrInsertComputed($rowSize, () => {
      ...
      const index = children.findIndex(
        (element) => element.offsetTop !== firstTop
      );
  ...
  attributeChangedCallback(__, prev, next) {
    ...
    const target =
      this.querySelector(Selectors.SlideById(next)) || this.children.item(0);

    if (!target) return;

    this.#selectedId = target.dataset.id;
    this.querySelector(Selectors.SlideById(prev))?.classList.remove('current');
    this.querySelector(Selectors.SlideById(this.#selectedId))?.classList.add(
      'current'
    );
  }
  ...
  #hopRow(delta) {
    ...
    const index = Number(
      this.querySelector(Selectors.SlideById(this.#selectedId))?.dataset.index
    );
